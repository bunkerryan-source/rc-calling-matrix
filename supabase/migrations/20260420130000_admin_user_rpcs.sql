-- Admin RPCs for managing user_access.
-- Both run as SECURITY DEFINER so authenticated users can read auth.users
-- and grant access without needing the service_role key. Each RPC re-checks
-- that the caller already has access to the ward before doing anything.
...
begin
  if not exists (
    ...
    where user_id = auth.uid()
      and ward_id = '00000000-0000-0000-0000-000000000001'::uuid
  ) then
